...
class _LoginViewState extends State<LoginView> {
  ...
  void _handleLogin() {
    ...
    if (result == "OK") {
      // Mock Data Dinamis untuk Modul 5: Gatekeeper Testing
      String assignedRole = 'Anggota';
      String assignedUid = '${user}_uid_999';
      
      if (user.toLowerCase() == 'admin') {
        assignedRole = 'Ketua';
        assignedUid = 'admin_uid_123';
      } else if (user.toLowerCase() == 'dzakir') {
        assignedRole = 'Anggota';
        assignedUid = 'dzakir_uid_456';
      }

      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => LogView(currentUser: {
            'uid': assignedUid,
            'username': user,
            'role': assignedRole,
            'teamId': 'MEKTRA_KLP_01', // Tim yang sama agar bisa melihat data satu sama lain
          }),
        ),
      );
    } else {
